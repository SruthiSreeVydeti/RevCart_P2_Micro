...
export class CartService {
  ...
  constructor(private http: HttpClient, private authService: AuthService, private apiConfig: ApiConfigService) {
    this.apiUrl = 'http://localhost:8090/api/cart';
  ...
  addToCart(product: any, quantity: number = 1): void {
    ...
    this.http.post(`${this.apiUrl}/${userId}/add`, cartRequest).subscribe({
      next: (response) => {
      ...
      next: (response: any) => {
        ...
        if (items.length > 0) {
          const productRequests = items.map((item: any) => 
            this.http.get(`http://localhost:8090/api/products/${item.productId}`)
          );
  ...
  removeFromCart(productId: number, size?: string): void {
    const userId = this.getUserId();
    this.http.delete(`${this.apiUrl}/${userId}/remove/${productId}`).subscribe({
  ...
  clearCart(): void {
    const userId = this.getUserId();
    this.http.delete(`${this.apiUrl}/${userId}/clear`).subscribe({
      next: () => {
        this.cartItems.next([]);
      },
